...
**Inject and extract** for the mesh use a **different** service: the **proxy backend** per [proxy-otel-api.md](./proxy-otel-api.md) (for example **`https://o.softprobe.ai`**). Envoy/WASM does **not** call this JSON API for request-path lookup.
...
4. proxy calls `/v1/inject` on the **proxy backend** using OTLP trace payloads (not this JSON API)
5. the **proxy backend** resolves the lookup using session/case state that stays **consistent** with this API (exact sync mechanism is product-specific — see `docs/design.md` open questions)

This keeps the user-facing control surface ergonomic while the mesh data plane uses protobuf to a **separate** inject/extract service.
