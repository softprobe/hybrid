...
- **Release hygiene** — Apache-2.0 `LICENSE` in every package, runtime container image and WASM OCI bundle on `ghcr.io/softprobe/*`, build-time CLI version string, and automated publish workflows for npm / PyPI / Maven Central / the Go module path. Target: **v0.6**. Tracks [tasks.md PD5](https://github.com/softprobe/softprobe/blob/main/tasks.md#phase-pd5--release-hygiene).
