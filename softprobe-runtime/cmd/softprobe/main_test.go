...
func TestRunDoctorJSONReportsSpecSchemaDrift(t *testing.T) {
	...
	if resp.ExpectedSpecVersion != controlapi.SpecVersion || resp.ExpectedSchemaVersion != controlapi.SchemaVersion {
		t.Fatalf("expected values = spec %q schema %q, want %q / %q", resp.ExpectedSpecVersion, resp.ExpectedSchemaVersion, controlapi.SpecVersion, controlapi.SchemaVersion)
	}
}
