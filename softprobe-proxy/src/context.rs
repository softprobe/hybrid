...
use crate::http_helpers::{
    build_extract_backend_request, build_inject_backend_request,
    classify_inject_backend_response, InjectBackendResponseDisposition,
};
...
impl SpHttpContext {
   pub fn new(context_id: u32, config: Config) -> Self {
    ...
    // Dispatch injection HTTP call to the configured proxy backend.
    fn dispatch_injection_lookup(&mut self) -> Result<u32, String> {
        crate::sp_debug!(
            "Starting injection lookup (host={:?}, path={:?}, backend={})",
            self.url_host,
            self.url_path,
            self.config.sp_backend_url
        );
        ...
        let otel_data = serialize_traces_data(&traces_data).map_err(|e| {
            let error_msg = format!("SP Injection: Serialization error: {}", e);
            crate::sp_error!("{}", error_msg);
            error_msg
        })?;

        let request = build_inject_backend_request(
            &self.config.sp_backend_url,
            &self.config.public_key,
            otel_data.len(),
        );
        let http_headers: Vec<(&str, &str)> = request
            .headers
            .iter()
        ...
        match self.dispatch_http_call(
            ...
            Ok(call_id) => {
                crate::sp_info!("Injection lookup dispatched successfully (call_id={})", call_id);
                Ok(call_id)
            }
            Err(status) => {
                let error_msg = format!("SP Injection: Failed to dispatch HTTP call, status: {:?}", status);
                crate::sp_error!("{}", error_msg);
                Err(error_msg)
            }
        }
    ...
    fn dispatch_async_extraction_save(&mut self) {
        ...
        let request = build_extract_backend_request(
            &self.config.sp_backend_url,
            &self.config.public_key,
            otel_data.len(),
        );
    ...
    fn on_http_call_response(
        ...
        if let Some(pending_token) = self.pending_inject_call_token {
            if pending_token == token_id {
                ...
                match classify_inject_backend_response(status_code) {
                    InjectBackendResponseDisposition::Hit if body_size > 0 => {
