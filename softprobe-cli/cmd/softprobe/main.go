...
func run(args []string, stdout, stderr io.Writer) int {
	...
	case "--version", "version":
		_, _ = fmt.Fprintf(rawStdout, "softprobe %s\n", version.CLIDetail(apiversion.SpecVersion))
		return exitOK
	case "--help", "-h", "help":
		printUsage(rawStdout)
		return exitOK
...
func runDoctor(args []string, stdout, stderr io.Writer) int {
	...
	if meta.SpecVersion != apiversion.SpecVersion || meta.SchemaVersion != apiversion.SchemaVersion {
		fields := map[string]any{
			"runtimeHealthy":        health.Status == "ok",
			"runtimeVersion":        meta.RuntimeVersion,
			"specVersion":           meta.SpecVersion,
			"schemaVersion":         meta.SchemaVersion,
			"expectedSpecVersion":   apiversion.SpecVersion,
			"expectedSchemaVersion": apiversion.SchemaVersion,
		}
		if *jsonOutput {
	...
	checks := []doctorCheckResult{
		...
		{Name: "schema-version", Status: "ok", Details: map[string]any{"expected": apiversion.SchemaVersion, "got": meta.SchemaVersion}},
		wasmCheck,
		echoCheck,
	}
	...
	if *jsonOutput {
		writeJSONEnvelope(stdout, "ok", 0, map[string]any{
			"cliVersion":     version.CLIDetail(apiversion.SpecVersion),
			"runtimeHealthy": health.Status == "ok",
			"runtimeVersion": meta.RuntimeVersion,
			"specVersion":    meta.SpecVersion,
			"schemaVersion":  meta.SchemaVersion,
...
func runSessionStart(args []string, stdout, stderr io.Writer) int {
	...
	if *jsonOutput {
		writeJSONEnvelope(stdout, "ok", exitOK, map[string]any{
			...
			"mode":            *mode,
			"schemaVersion":   apiversion.SchemaVersion,
			"specVersion":     apiversion.SpecVersion,
		})
		return exitOK
	}
	if *shellOutput {
