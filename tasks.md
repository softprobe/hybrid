...
- [x] **PD5.3b — Proxy WASM OCI bundle on ghcr.** Added [`softprobe-proxy/Dockerfile.ghcr-wasm`](softprobe-proxy/Dockerfile.ghcr-wasm) (scratch + `/plugin.wasm`), [`.dockerignore`](softprobe-proxy/.dockerignore), and [`.github/workflows/softprobe-proxy-wasm-image.yml`](.github/workflows/softprobe-proxy-wasm-image.yml): Rust `wasm32-unknown-unknown` build via `make build`, then Buildx — on **PR** load + `docker cp`/`file` smoke; on **push** GHCR tags (`sha-*`, `latest`, git tag) + **`oras copy` from GHCR to `oci:` layout** and scan for `\0asm` magic. [`docs-site/deployment/kubernetes.md`](docs-site/deployment/kubernetes.md) tip documents `oci://` + ORAS verify; [`softprobe-proxy/README.md`](softprobe-proxy/README.md#wasm-oci-image-ghcr) has full `WasmPlugin` snippet.
  **Verify:** PR touching `softprobe-proxy/**` runs workflow green; after merge, `crane export ghcr.io/<owner>/softprobe-proxy:latest | tar -xO plugin.wasm > plugin.wasm` and `oras copy ghcr.io/<owner>/softprobe-proxy:latest oci:./out` + `\0asm` scan succeed; `WasmPlugin.url: oci://ghcr.io/<owner>/softprobe-proxy:…` matches published tags.
